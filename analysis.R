...
vnames = sapply(sTS,"[[","paleoData_variableName")
...
for(i in c(1:5,7,9:17)){
  ...
  sumStats = as.tibble(apply(all.cp,2,quantile,probs = quants))
  sumStats$quantiles =quants
  write_csv(x = sumStats,path = here("figures",paste0(dsn[i],"-",sTS[[i]]$paleoData_variableName,"-summaryStats.csv")))
  
  #store as a list
  allDist[[paste0(dsn[i],"-",sTS[[i]]$paleoData_variableName)]] <- all.cp
  
  
  #force to be even number of columns
  if(ncol(all.cp)%%2==1){#if odd number
    all.cp = cbind(all.cp,all.cp[,ncol(all.cp)])
    
  }
  
  
  tsPlot <- plotTimeseriesEnsRibbons(ae,y,colorLow = "LightGray",colorHigh = "DarkGray") +
    geom_line(aes(x = age.med ,y = y),color = "red") +
...
#ensCoevalProb(all.cp[,1],all.cp[,2],thresh= 100)
